...
//    }
    ...
    public void validateIKTCalculatorTest() throws InterruptedException {
        ...
        //weightInput.sendKeys(Keys.F1, Keys.ADD);
        calcButton.click();
    ...
    public void validateSKFTest() throws InterruptedException {
        ...
        Select selectSex = new Select(selectWebElement); //инициализация обертки
       /* selectSex.selectByIndex(1); // хранится как масив в drop daun
        Thread.sleep(2000);

        selectSex.selectByValue("0"); // тут стрингой указываем
        Thread.sleep(2000);

        selectSex.selectByVisibleText("женский");
        Thread.sleep(2000);*/

        // selectSex.deselectAll(); // очистит выбор
        ...
        driver.findElement(By.cssSelector("input[type=\"button\"][value=\"Рассчитать\"]")).click(); // сайт изменился не есть одинаковые элементы

        Assert.assertEquals(driver.findElement(By.id("txtMDRD")).getText(),
                "MDRD: 100 (мл/мин/1,73кв.м)");
